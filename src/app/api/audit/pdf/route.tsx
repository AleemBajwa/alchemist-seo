  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {technicalSummary && pagesAudited > 1 && (
          <View style={{ marginTop: 20 }}>
            <Text style={styles.issuesTitle}>Technical Summary</Text>
            <Text style={{ fontSize: 9, marginBottom: 8, color: "#666" }}>
              Crawl Source: {technicalSummary.crawlSource === "sitemap" ? "Sitemap" : "Link Discovery"}
            </Text>
            {technicalSummary.orphanPageUrls.length > 0 && (
              <Text style={{ fontSize: 9, marginBottom: 4, color: "#666" }}>
                Orphan Pages: {technicalSummary.orphanPageUrls.length}
              </Text>
            )}
            {technicalSummary.duplicateTitleGroups.length > 0 && (
              <Text style={{ fontSize: 9, marginBottom: 4, color: "#666" }}>
                Duplicate Title Groups: {technicalSummary.duplicateTitleGroups.length}
              </Text>
            )}
            {technicalSummary.duplicateMetaGroups.length > 0 && (
              <Text style={{ fontSize: 9, marginBottom: 4, color: "#666" }}>
                Duplicate Meta Groups: {technicalSummary.duplicateMetaGroups.length}
              </Text>
            )}
            {technicalSummary.redirectChainExamples.length > 0 && (
              <Text style={{ fontSize: 9, marginBottom: 4, color: "#666" }}>
                Redirect Chains Found: {technicalSummary.redirectChainExamples.length} examples
              </Text>
            )}
          </View>
        )}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { auditId, url, score, grade, issues, pagesAudited, technicalSummary } = body;
